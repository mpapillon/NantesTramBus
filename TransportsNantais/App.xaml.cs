...
namespace TransportsNantais
{
    ...
    {
        static SQLite.SQLiteAsyncConnection Connection;
        static ManualResetEvent DBLoaded = new ManualResetEvent(false);

        public static Task<SQLite.SQLiteAsyncConnection> GetDatabaseAsync()
        {
            return Task.Run(() =>
            {
                DBLoaded.WaitOne();
        ...
        public const string _packageDbPath = @"Resources\database.db";
        ...
        private void RootFrame_Navigating(object sender, NavigatingCancelEventArgs e)
        {
            ...
            RootFrame.Dispatcher.BeginInvoke(delegate
            {
                ...
                if (contains == false)
                {
                    RootFrame.Navigate(new Uri("/Views/FirstStart/WelcomeView.xaml", UriKind.Relative));
                }
                else
                {
                    if ((bool)IsolatedStorageSettings.ApplicationSettings["firstStart"])
                    {
                        RootFrame.Navigate(new Uri("/Views/FirstStart/WelcomeView.xaml", UriKind.Relative));
                    }
                    else
                    {
                        RootFrame.Navigate(new Uri("/Views/MainView.xaml", UriKind.Relative));
                    }
                }
            });
        }
